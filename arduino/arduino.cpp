...
void serial_parse(void) {
  /*
   * Assumes integer to parse is aaaabbbb, where
   * a - servo [0, 9999]
   * b - motor [0, 9999]
   */
  int parsed_int = Serial.parseInt();
  if (parsed_int >= 0) {
    serial_servo = parsed_int / 1e4;
    parsed_int -= serial_servo * 1e4;
...
void setup() {
  ...
  // Radio interrupts
  // attachPinChangeInterrupt(digitalPinToPinChangeInterrupt(PIN_RADIO_SWITCH),
    // interrupt_radio_switch, CHANGE);
...
void loop() {
  ...
  // Serial.print(radio_button_pwm);
  Serial.print(control_mode);
  Serial.print(", ");
  Serial.print(control_servo_pwm);
  Serial.print(", ");
  Serial.print(radio_servo_pwm);
  Serial.print(", ");
  Serial.print(control_motor_pwm);
  Serial.print(",");
  Serial.print(radio_motor_pwm);
  Serial.println("");
  delay(100);
}
